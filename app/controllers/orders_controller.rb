class OrdersController < ApplicationController
  ...
  def create
    ...
    if book.stock_quntity >= order_params[:stock_quntity].to_i
      ...
      order = book.orders.create(date_submit: Time.now, status: 1, subtotal: book.price, total:,
                                 account_id: current_account.id, stock_quntity: order_params[:stock_quntity])
      if !order.nil?
  ...
  def users_orders
    @user = if current_account.type == 'Admin'
              User.find(params[:book_id])
            else
